...
class BetterRagNoKeywordPrompt:
    def __init__(
        self,
        ...
        # cross-encoder to rerank relevance
    ...
    def rerank_chunks(self, query: str, chunks: List[str], top_k: int) -> List[str]:
        if len(chunks) == 0:
            return []

        cross_input = [[query, chunk] for chunk in chunks]
        relevance_scores = self.reranker.predict(cross_input)

        chunk_scores = list(zip(chunks, relevance_scores))
        chunk_scores.sort(key=lambda x: x[1], reverse=True) # descending scores
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        for _idx, interaction_id in enumerate(batch_interaction_ids):
            ...
            reranked_chunks = self.rerank_chunks(
                query,
                merged_chunks,
                top_k=NUM_RERANKED_SENTENCES
            )

            batch_retrieval_results.append(reranked_chunks)
